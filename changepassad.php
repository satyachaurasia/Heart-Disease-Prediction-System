...
<ul class="w3-navbar w3-light-grey ">
  ...
  <li><a  class=" w3-black" href="changepassad.php">Change Password</a></li>
</ul>
	<?php 
		...
		$chg_pwd=mysql_query("select * from admin where username='".$_SESSION['username'] ."'");
		$chg_pwd1=mysql_fetch_array($chg_pwd);
		$data_pwd=$chg_pwd1['password'];
		if($data_pwd==$old_pass){
		if($new_pass==$re_pass){
			$update_pwd=mysql_query("update admin set password='$new_pass' where username='".$_SESSION['username'] ."'");
			echo "<script>alert('Update Sucessfully'); window.location='admin.php'</script>";
		}
		else{
			echo "<script>alert('Your new and Retype Password is not match'); window.location='changepassad.php'</script>";
		}
		}
		else
		{
		echo "<script>alert('Your old password is wrong'); window.location='changepassad.php'</script>";
		}}
	?>
